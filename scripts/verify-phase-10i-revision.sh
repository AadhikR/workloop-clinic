...
verify_revision() {
  compose run --rm --no-deps \
    ...
    /verification/verify-phase-10i-revision.py "$1"
}
...
compose run --rm migrate alembic -c /app/alembic.ini downgrade c6e8a1b3d927
...
compose run --rm migrate alembic -c /app/alembic.ini upgrade c6e8a1b3d927
test "$(verify_revision head)" = "$head_hash"
restore_head
trap - EXIT HUP INT TERM

echo "Phase 10I exact predecessor rollback and replay passed."
