...
revision_hash() {
  compose run --rm --no-deps \
    ...
    /verification/verify-phase-9d-revision.py "$1"
}
...
compose run --rm migrate alembic -c /app/alembic.ini downgrade a1c3e5f7b9d2
...
echo "Phase 9D exact predecessor rollback and replay passed."
