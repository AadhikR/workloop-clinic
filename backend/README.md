...
This directory contains the portable Python backend that will replace direct browser access to Supabase. Phase 2B created the package and dependency foundation. Phase 2C added an isolated local PostgreSQL service without connecting application code to it. FastAPI endpoints, application database connections, migrations, and Keycloak integration are added in later parts.
...
- Do not add an application database connection until Phase 2D.
- Do not add FastAPI routes until Phase 2D.
