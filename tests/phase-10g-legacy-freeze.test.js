...
test('legacy roster draft writers fail closed after Phase 10G cutover', () => {
  for (const symbol of ['saveRosterAssignment', 'deleteRosterAssignment']) {
    const body = functionBody(attendance, symbol)
    assert.match(body, /moved to the migration roster screen/i)
