...
**`generateExpiryNotifications`** is called once at the end of the Dashboard's `Promise.all` data load — runs async after `setLoading(false)`, silently swallows errors. Signature: `generateExpiryNotifications(employees, documents, insurancePolicies, allCertifications = [], allEmployeeDocs = [])`. Generates: document/visa/passport expiry alerts for the admin; `clinical_credential_expiry` alerts at 90d/30d/14d for clinical credential document types (DHA Licence, DOH/MOH Licence, BLS/ACLS/PALS/NRP/CME — defined in module-private `CLINICAL_DOC_TYPES` Set within `notificationStorage.js`); insurance renewal alerts; `cert_expiry` alerts for certifications; `probation_ending` and `contract_expiry` alerts. All use ON CONFLICT DO NOTHING deduplication with `{id}_{thr}d` related_entity_id suffix.
