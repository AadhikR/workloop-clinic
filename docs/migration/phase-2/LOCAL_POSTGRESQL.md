...
Create the ignored password file once from the repository root:

```powershell
& ".\scripts\new-local-postgres-env.ps1"
```

The script refuses to overwrite an existing file. It generates four unique 256-bit values
and does not display them. Store a protected copy outside Git if the local database must be
recoverable after losing this file.
